...
module.exports = (client) => {
    ...
    (async () => {
        try {            logger.info(`Started refreshing ${isDevelopment ? 'development' : 'global'} application (/) commands.`);
            logger.debug(`Found ${commands.length} commands to register`);

            let data;
            if (isDevelopment) {
                // In development mode, register commands only to development server
                logger.debug(`Registering commands to development guild: ${process.env.DEV_GUILD_ID}`);
                data = await rest.put(
                    Routes.applicationGuildCommands(process.env.CLIENT_ID, process.env.DEV_GUILD_ID),
                    { body: commands },
                );
                logger.info(`Successfully reloaded ${data.length} development commands for guild ${process.env.DEV_GUILD_ID}`);
            } else {
